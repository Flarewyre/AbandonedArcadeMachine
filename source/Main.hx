...
class Main extends Sprite
{
	...
	public static var scanlinesShader:ScanlinesShader;
	public static var vhsShader:VhsShader;
	public static var scanlinesFilter:ShaderFilter;
	...
	**/
	public static var gameWeeks:Array<Dynamic> = [
		[
			['Family-Diner', 'Slaughter', 'Massacre', 'Happiest-Day'],
			['springbon', 'william', 'william', 'bonnie'],
	...
	public function new()
	{
		...
		**/
		...
		scanlinesShader = new ScanlinesShader();
		vhsShader = new VhsShader();
		scanlinesFilter = new ShaderFilter(scanlinesShader);
		shaderFilter = new ShaderFilter(vhsShader);
		...
		// var scanlines = new FlxSprite().loadGraphic(Paths.image('UI/base/Scanlines'));
		// scanlines.antialiasing = true;
		// scanlines.scrollFactor.set(1, 1);
		// scanlines.setGraphicSize(Std.int(scanlines.width * 2), Std.int(scanlines.height * 2));
	...
	{
		vhsShader.glitchPower.value[0] = 0.15;
