...
class VhsShader extends FlxShader
{
	@:glFragmentSource('
		...
		vec3 getVideo(vec2 uv)
		{
			...
			look.x = look.x + (sin(look.y*10. + iTime)/50.*onOff(4.,4.,.3)*(1.+cos(iTime*80.))*window)*glitchPower;
			float vShift = 0.4*onOff(2.,3.,.9) * (sin(iTime)*sin(iTime*20.0) + (0.5 + 0.1*sin(iTime*200.0)*cos(iTime)));
			look.y = mod(look.y, 1.);
		...
		void main()
        {
			...
			gl_FragColor = vec4(video, gl_FragColor.a);
        }')
	public function new()
	{
		super();
		iTime.value = [0.0];
		glitchPower.value = [0.15];
